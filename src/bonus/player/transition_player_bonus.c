...
/*   Updated: 2024/02/27 23:47:00 by hoatran          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	transition_player(t_game *game, keys_t key)
{
	...
	if (player->state == PLAYER_IDLE)
	{
		if (key == MLX_KEY_W
			...
			return (set_player_state(player, PLAYER_MOVE_UP));
		if (key == MLX_KEY_D
			&& is_movable(game, player->x + RENDER_PIXELS, player->y))
			return (set_player_state(player, PLAYER_MOVE_RIGHT));
		if (key == MLX_KEY_S
			&& is_movable(game, player->x, player->y + RENDER_PIXELS))
			return (set_player_state(player, PLAYER_MOVE_DOWN));
		if (key == MLX_KEY_A
			&& is_movable(game, player->x - RENDER_PIXELS, player->y))
			return (set_player_state(player, PLAYER_MOVE_LEFT));
	}
}
